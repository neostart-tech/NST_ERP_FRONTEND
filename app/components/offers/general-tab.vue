		...
		<div class="bg-blue-50 rounded-lg p-4">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				...
				<div>
					...
					<input :value="parseForDateInput(offerFormData.publication_date)"
						@input="offerFormData.publication_date = ($event.target as HTMLInputElement).value" type="date" :class="[
							'w-full rounded-md px-3 py-2 border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500',
				...
				<div>
					...
					<input :value="parseForDateTimeInput(offerFormData.submission_deadline)"
						@input="offerFormData.submission_deadline = ($event.target as HTMLInputElement).value" type="datetime-local"
						:class="[
							'w-full rounded-md px-3 py-2 border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500',
				...
				<div>
					...
					<input :value="parseForDateInput(offerFormData.file_obtaining_deadline)"
						@input="offerFormData.file_obtaining_deadline = ($event.target as HTMLInputElement).value" type="date" :class="[
						'w-full rounded-md px-3 py-2 border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500',
				...
				<div>
					...
					<input :value="parseForDateTimeInput(metadataFormData.tender_opening_date)"
						@input="metadataFormData.tender_opening_date = ($event.target as HTMLInputElement).value"
						type="datetime-local" :class="[
							'w-full rounded-md px-3 py-2 border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500',
				...
				<div>
					...
					<div class="flex items-center space-x-6">
						<label class="inline-flex items-center space-x-2">
							<input type="radio" class="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500" value="1"
								v-model="metadataFormData.has_bank_guarentee" name="bank_guarantee" />
					...
					<div v-if="metadataFormData.has_bank_guarentee == 1" class="mt-3">
						<label class="block text-sm font-medium text-gray-700 mb-2">
							Montant de la garantie
							<RequiredField />
						</label>
						<input v-model.number="metadataFormData.bank_guarentee" type="number" min="0"
							class="w-full rounded-md px-3 py-2 border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
							placeholder="Ex: 500000" />
					</div>
					<p v-if="validationErrors.bank_guarentee" class="text-red-500 text-sm mt-1">
						{{ validationErrors.bank_guarentee }}
					</p>
				</div>
			</div>
		</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					...
					<select v-model="offerFormData.enterprise_id" :class="[
						...
						<option value="">Sélectionner une autorité contractante</option>
				...
				<div>
					...
					<select v-model="offerFormData.offer_type_id"
						class="w-full rounded-md px-3 py-2 border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
					...
					<p v-if="validationErrors.offer_type_id" class="text-red-500 text-sm mt-1">
						{{ validationErrors.offer_type_id }}
					</p>
				</div>
				<div>
					<label class="block text-sm font-medium text-gray-700 mb-2">
...
import { parseForDateTimeInput } from "~/app/utils/dateParser";
...
const offerStore = useOfferTypeStore();
...
onMounted(async () => {
	...
	await offerStore.fetchOfferTypes();
	offerTypes.value = offerStore.offerTypes;
