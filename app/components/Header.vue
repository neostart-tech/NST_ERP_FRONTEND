				...
				<button @click="showUserMenu = !showUserMenu"
					...
					<div class="relative">
						<div
							...
							<span>
								{{
									authStore.fullName.charAt(0).toLocaleUpperCase() || "U"
								}}
							</span>
						</div>
					</div>
					<div class="text-left">
						<p class="text-sm font-medium text-gray-800">
							{{ authStore.fullName || "Utilisateur" }}
						</p>
						<p class="text-xs text-gray-500">
							{{
								user?.loggedInAt ?
									formatRelativeDate(user?.loggedInAt) :
									""
							}}
						</p>
					</div>
...
import { getPageContentTitle } from "../utils/headerConfigs";
...
const pageTitle = computed(() => {
	...
	return getPageContentTitle(route.name as string);
});
