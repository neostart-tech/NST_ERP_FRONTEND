		...
		<div class="relative">
			...
			>
				<input
					...
					@input="isOpen = true"
					type="text"
					:placeholder="selectedClient ? getClientName(selectedClient) : placeholder"
			...
			>
				...
				>
					...
					<div
						...
						:class="{ 'bg-blue-50': selectedClient?.id === client.id }"
					>
						<!-- Client Info -->
						<div class="flex-1 min-w-0">
							<p class="text-gray-900 font-medium truncate">{{ getClientName(client) }}</p>
							<div class="flex items-center gap-2 text-sm text-gray-500">
								<span v-if="showEmail && client.email" class="truncate">{{ client.email }}</span>
								<span v-if="showPhone && client.phone" class="truncate">{{ client.phone }}</span>
								<span v-if="showType" class="text-xs bg-gray-200 px-2 py-0.5 rounded">{{ client.client_type }}</span>
							</div>
						</div>
...
const props = withDefaults(defineProps<{
	...
	onUpdateModelValue?: (value: string | Client | null) => void
}>(), {
	modelValue: null,
	label: '',
...
const emit = defineEmits<{
	(e: 'update:modelValue', value: string | Client | null): void
...
const filteredClients = computed(() => {
	...
	return props.clients.filter((client: Client) => {
...
watch(() => props.modelValue, (newValue) => {
	...
	if (typeof newValue === 'string') {
		// Si c'est un ID, trouver le client correspondant
		const client = props.clients.find(c => c.id === newValue)
		if (client) {
...
// Watch pour mettre à jour quand la liste des clients change
watch(() => props.clients, () => {
...
const selectClient = (client: Client) => {
	...
	const emittedValue = props.returnObject ? client : client.id
