<template>
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
			...
			>
				<div class="flex items-center justify-between">
					...
					<div
						class="w-12 h-12 bg-emerald-100 rounded-xl flex items-center justify-center ml-2 flex-shrink-0"
					>
						<svg
			...
			>
				<div class="flex items-center justify-between">
					...
					<div
						class="w-12 h-12 bg-violet-100 rounded-xl flex items-center justify-center"
					>
						<svg
		...
		<div
			class="bg-white rounded-xl shadow overflow-hidden border border-gray-100 mb-8"
		>
			...
			<div v-if="!gridView" class="overflow-x-auto">
				<table class="min-w-full divide-y divide-gray-200">
					<thead class="bg-gray-50">
						<tr>
							<th
								class="px-3 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider"
							>
								Facture
							</th>
							<th
...
const handleClickOutside = (event) => {
	if (
		activeScheduleActions.value &&
		!event.target.closest(".schedule-actions")
	) {
...
const toggleScheduleActions = (scheduleId) => {
	activeScheduleActions.value =
		activeScheduleActions.value === scheduleId ? null : scheduleId;
};
...
const calculateStats = () => {
	...
	stats.value = {
		totalSchedules: paymentSchedules.value.length,
		totalDue: paymentSchedules.value.reduce((sum, s) => sum + s.amount, 0),
		overdueInvoices: paymentSchedules.value.filter(
			(s) => new Date(s.dueDate) < today
		).length,
		overduePercentage:
			paymentSchedules.value.length > 0
				? Math.round(
						(paymentSchedules.value.filter((s) => new Date(s.dueDate) < today)
							.length /
							paymentSchedules.value.length) *
							100
				  )
				: 0,
		totalReminders: paymentSchedules.value.filter((s) => s.reminderSent).length,
	};
};
...
const sendReminderEmail = async (
	...
	try {
		const emailData = {
			...
			subject: `Rappel de paiement - Facture ${
				schedule.invoice?.reference || ""
			}`,
			body: `Bonjour ${
				schedule.invoice?.order?.client?.name || "Client"
			},<br><br>
            Un paiement de ${formatCurrency(
							installment.amount
						)} était dû le ${formatDate(installment.date)}.<br>
