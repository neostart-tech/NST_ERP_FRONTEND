    ...
    <div class="ml-0 p-10">
      ...
      <main class="mx-auto max-w-7xl px-5">
        ...
        <div class="flex justify-between items-center mb-8">
          ...
          <button @click="showForm = true" class="bg-gradient-to-r from-blue-500 to-green-500 hover:from-blue-600 hover:to-green-600 text-white px-6 py-3 rounded-lg flex items-center shadow-md hover:shadow-lg transition-all">
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-blue-50 to-green-50">
                <tr>
                  ...
                  <th class="px-4 py-4 text-left text-xs font-semibold text-blue-800 uppercase tracking-wider">Équipement</th>
                  <th class="px-4 py-4 text-left text-xs font-semibold text-blue-800 uppercase tracking-wider">Date signalement</th>
                  <th class="px-4 py-4 text-left text-xs font-semibold text-blue-800 uppercase tracking-wider">Technicien</th>
                  <th class="px-4 py-4 text-left text-xs font-semibold text-blue-800 uppercase tracking-wider">Statut</th>
                  <th class="px-4 py-4 text-left text-xs font-semibold text-blue-800 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-100">
                <tr v-for="report in reports" :key="report.id" class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(report.report_date) }}</td>
                  <td class="px-4 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center">
                        <span class="text-blue-600 font-medium text-xs">{{ report.technician_name ? report.technician_name.split(' ').map(n => n[0]).join('') : '' }}</span>
                      </div>
                      <div class="ml-3">
                  ...
                  <td class="px-4 py-4 whitespace-nowrap">
                    <span :class="`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(report.status)}`">
                      {{ report.status }}
                    </span>
                  </td>
                  <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                    <div class="flex space-x-2">
                      ...
                      <button @click="editReport(report)" class="text-yellow-600 hover:text-yellow-800 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                        Modifier
                      </button>
                      <button @click="deleteReport(report.id)" class="text-red-600 hover:text-red-800 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-screen overflow-y-auto">
        <div class="p-6">
          <div class="flex justify-between items-center border-b pb-4">
            <h2 class="text-2xl font-bold text-gray-800">{{ editing ? 'Modifier le signalement' : 'Nouveau signalement' }}</h2>
          ...
          <form @submit.prevent="submitForm" class="mt-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Sélectionner l'équipement <span class="text-red-500">*</span></label>
                <select v-model="form.equipement_id" @change="loadEquipmentDetails" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                  <option value="">Sélectionner un équipement</option>
                  <option v-for="equip in equipmentList" :key="equip.id" :value="equip.id">{{ equip.brand }} {{ equip.model }} ({{ equip.serial_number }})</option>
                </select>
              </div>
              
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Sélectionner le client <span class="text-red-500">*</span></label>
                <select v-model="form.client_id" @change="loadClientDetails" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div v-if="selectedEquipment" class="bg-blue-50 p-4 rounded-lg border border-blue-100">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p class="font-medium" :class="selectedEquipment.under_warranty ? 'text-green-600' : 'text-red-600'">{{ selectedEquipment.under_warranty ? 'Oui' : 'Non' }}</p>
                </div>
              </div>
            </div>
            ...
            <div v-if="selectedClient" class="bg-green-50 p-4 rounded-lg border border-green-100">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p class="font-medium">{{ selectedClient.type === 'company' ? 'Client moral (entreprise)' : 'Client physique' }}</p>
                </div>
                <div>
                  <p class="text-xs text-gray-600">{{ selectedClient.type === 'company' ? 'Nom de l\'entreprise' : 'Nom du client' }}</p>
                  <p class="font-medium">{{ selectedClient.type === 'company' ? selectedClient.company_name : selectedClient.first_name + ' ' + selectedClient.last_name }}</p>
                </div>
                <div>
                  <p class="text-xs text-gray-600">Téléphone</p>
                  <p class="font-medium">{{ selectedClient.telephone_number }}</p>
                </div>
                <div>
            ...
            <!-- Problem Description -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Défaut constaté <span class="text-red-500">*</span></label>
              <textarea v-model="form.problem_description" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></textarea>
            </div>
            <!-- Technician Assignment -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Technicien assigné <span class="text-red-500">*</span></label>
                 <select
  id="technicien_id"
...
>
  ...
  <option
    v-for="technicien in technicians"
    :key="technicien.id"
    :value="technicien.id"
  >
    {{ technicien.full_name }}
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Date du signalement <span class="text-red-500">*</span></label>
                <input v-model="form.report_date" type="datetime-local" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Heure de contact</label>
                <input v-model="form.contact_time" type="time" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select v-model="form.status" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              ...
              <div class="space-y-3">
                <div class="flex items-center">
                  <input v-model="form.is_tested_certified" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                  <label class="ml-2 block text-sm text-gray-700">L'objet testé certifié en état de marche</label>
                </div>
                <div class="flex items-center">
                  <input v-model="form.is_sent_for_maintenance" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                ...
                <div v-if="form.is_sent_for_maintenance" class="flex items-center ml-6">
                  <input v-model="form.needs_delivery" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                ...
                <div v-if="form.needs_delivery" class="ml-6">
                  ...
                  <div class="relative rounded-md shadow-sm">
                    ...
                    <input v-model="form.delivery_price" type="number" class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-lg px-4 py-2">
                  </div>
                </div>
                <div class="flex items-center">
                  <input v-model="form.under_contract" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <input v-model="form.repair_start_date" type="datetime-local" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Date fin réparation</label>
                <input v-model="form.repair_end_date" type="datetime-local" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Durée de réparation</label>
                <input v-model="form.repair_duration" type="text" readonly class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-100">
              </div>
            </div>
            <!-- Technician Findings -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Défaut constaté par le technicien</label>
              <textarea v-model="form.technician_findings" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></textarea>
            </div>
            ...
            <div class="border-t pt-6 mt-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div>
                  ...
                  <div class="flex items-center">
                    ...
                    <button @click="$refs.quotationFile.click()" type="button" class="bg-blue-50 hover:bg-blue-100 text-blue-700 px-4 py-2 rounded-lg border border-blue-200 flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" />
                      </svg>
                      Téléverser
                    </button>
                    <span v-if="form.quotation_file_name" class="ml-3 text-sm text-gray-600">{{ form.quotation_file_name }}</span>
                  </div>
                </div>
                
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">Montant du devis (FCFA)</label>
                  <input v-model="form.quotation_amount" type="number" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div class="flex items-center">
                    <button @click="toggleAudioRecording" type="button" :class="`px-4 py-2 rounded-lg border flex items-center ${isRecording ? 'bg-red-100 border-red-300 text-red-700' : 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100'}`">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div>
                  ...
                  <div class="flex items-center">
                    <input v-model="form.quotation_email_sent" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                    <label class="ml-2 block text-sm text-gray-700">Devis envoyé par email</label>
                  </div>
                  <input v-if="form.quotation_email_sent" v-model="form.quotation_email_date" type="datetime-local" class="w-full mt-2 border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
              </div>
              
              <div class="mt-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Commentaires sur le devis</label>
                <textarea v-model="form.quotation_comments" rows="2" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></textarea>
              </div>
            </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div class="border border-gray-300 rounded-lg h-32 bg-gray-50 flex items-center justify-center">
                    <img v-if="form.technician_signature" :src="form.technician_signature" alt="Signature technicien" class="max-h-28 max-w-full">
                    <button v-else type="button" @click="showSignatureModal('technician')" class="text-blue-600 hover:text-blue-800 flex items-center">
                ...
                <div>
                  ...
                  <div class="border border-gray-300 rounded-lg h-32 bg-gray-50 flex items-center justify-center">
                    <img v-if="form.client_signature" :src="form.client_signature" alt="Signature client" class="max-h-28 max-w-full">
                    <button v-else type="button" @click="showSignatureModal('client')" class="text-blue-600 hover:text-blue-800 flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                      </svg>
                      Ajouter signature
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t">
              <button type="button" @click="closeForm" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-6 py-2 rounded-lg transition-colors">
                Annuler
              </button>
              <button type="submit" class="bg-gradient-to-r from-blue-500 to-green-500 hover:from-blue-600 hover:to-green-600 text-white px-6 py-2 rounded-lg shadow-md hover:shadow-lg transition-all">
    ...
    <div v-if="showSignaturePad" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl p-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <button @click="showSignaturePad = false" class="text-gray-500 hover:text-gray-700 rounded-full p-1 hover:bg-gray-100">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        ...
        <div class="flex justify-between">
          <button @click="clearSignature" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg transition-colors">
            Effacer
          </button>
          <button @click="saveSignature" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors">
    ...
    <div v-if="viewingReport" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 print-modal">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-screen overflow-y-auto print:w-full print:max-w-none print:shadow-none print:rounded-none print:overflow-visible">
        <div class="p-8 print:p-6">
          ...
          <div class="flex justify-between items-start mb-10 print:mb-8">
            ...
            <div class="text-right">
              <div class="bg-gradient-to-r from-blue-500 to-green-500 text-white px-6 py-3 rounded-lg inline-block shadow-lg">
          ...
          <div v-if="editingCompanyInfo" class="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
            ...
            <div class="flex justify-end mt-4 space-x-3">
              <button @click="editingCompanyInfo = false" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg">
                Annuler
              </button>
          ...
          <div class="bg-gradient-to-r from-blue-50 to-green-50 p-6 rounded-xl mb-10 print:mb-8 shadow-sm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <span :class="`px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full ${getStatusColor(viewingReport.status)}`">
          ...
          <div class="mb-10 print:mb-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-blue-500 inline-block">Détails de l'équipement</h2>
          ...
          <div class="mb-10 print:mb-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-green-500 inline-block">Informations client</h2>
            <div class="bg-green-50 p-6 rounded-xl shadow-sm">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-4 rounded-lg shadow-sm">
                  <p class="text-sm text-green-600 font-medium mb-1">Client</p>
                  <p class="font-medium text-gray-800">{{ viewingReport.client_name || viewingReport.client_company_name }}</p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm">
          ...
          <!-- Description du problème -->
            <div class="mb-10 print:mb-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-purple-500 inline-block">Description du problème</h2>
          ...
          <div class="mb-10 print:mb-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-yellow-500 inline-block">Informations sur le technicien</h2>
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <div class="flex items-center mt-1">
                    <div class="flex-shrink-0 h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                      <span class="text-blue-600 font-medium text-sm">{{ viewingReport.technician_name ? viewingReport.technician_name.split(' ').map(n => n[0]).join('') : '' }}</span>
                    </div>
                    <div class="ml-3">
          ...
          <div class="mb-10 print:mb-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-indigo-500 inline-block">Détails de la réparation</h2>
          ...
          <!-- Status Flags -->
          <div class="mb-10 print:mb-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-pink-500 inline-block">État de l'intervention</h2>
            <div class="bg-pink-50 p-6 rounded-xl shadow-sm">
              <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="bg-white p-4 rounded-lg shadow-sm flex items-center">
                  <span :class="`h-4 w-4 rounded-full ${viewingReport.is_tested_certified ? 'bg-green-500' : 'bg-gray-300'}`"></span>
                  <span class="ml-3 text-sm font-medium text-gray-800">Testé et certifié</span>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm flex items-center">
                  <span :class="`h-4 w-4 rounded-full ${viewingReport.is_sent_for_maintenance ? 'bg-green-500' : 'bg-gray-300'}`"></span>
                  <span class="ml-3 text-sm font-medium text-gray-800">Envoyé pour maintenance</span>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm flex items-center">
                  <span :class="`h-4 w-4 rounded-full ${viewingReport.needs_delivery ? 'bg-green-500' : 'bg-gray-300'}`"></span>
                  <span class="ml-3 text-sm font-medium text-gray-800">Livraison demandée</span>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm flex items-center">
                  <span :class="`h-4 w-4 rounded-full ${viewingReport.under_contract ? 'bg-green-500' : 'bg-gray-300'}`"></span>
            ...
            <div class="bg-blue-50 p-6 rounded-xl shadow-sm">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <p class="font-medium text-gray-800">{{ viewingReport.quotation_amount ? `${viewingReport.quotation_amount} FCFA` : 'Non spécifié' }}</p>
                </div>
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <div v-if="viewingReport.audio_recording_path" class="mt-2">
                    <button @click="downloadAudio(viewingReport.id)" class="text-blue-600 hover:text-blue-800 flex items-center">
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <div class="flex items-center">
                    <span :class="`h-4 w-4 rounded-full ${viewingReport.quotation_email_sent ? 'bg-green-500' : 'bg-gray-300'}`"></span>
                    <span class="ml-3 text-sm font-medium text-gray-800">
                      {{ viewingReport.quotation_email_sent ? `Oui (${formatDate(viewingReport.quotation_email_date)})` : 'Non' }}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <!-- Signatures -->
          <div class="mb-10 print:mb-8">
            <h2 class="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-teal-500 inline-block">Signatures</h2>
            <div class="bg-teal-50 p-6 rounded-xl shadow-sm">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-white p-6 rounded-lg shadow-sm text-center">
                  ...
                  <div class="border-2 border-dashed border-gray-300 rounded-lg h-32 bg-gray-50 flex items-center justify-center mb-3">
                    <img v-if="viewingReport.technician_signature" :src="viewingReport.technician_signature" alt="Signature technicien" class="max-h-28 max-w-full">
                ...
                <div class="bg-white p-6 rounded-lg shadow-sm text-center">
                  ...
                  <div class="border-2 border-dashed border-gray-300 rounded-lg h-32 bg-gray-50 flex items-center justify-center mb-3">
                    <img v-if="viewingReport.client_signature" :src="viewingReport.client_signature" alt="Signature client" class="max-h-28 max-w-full">
                    <span v-else class="text-gray-400">Non signé</span>
                  </div>
                  <p class="text-sm font-medium text-gray-800">{{ viewingReport.client_name || viewingReport.client_company_name }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="mt-12 pt-6 border-t border-gray-200 text-center text-sm text-gray-600 print:mt-10">
            ...
            <p class="mt-1">Email: {{ companyInfo.email }} <span v-if="companyInfo.website">| Site web: {{ companyInfo.website }}</span></p>
          </div>
          
          <!-- Boutons d'action (non imprimables) -->
          <div class="flex justify-end space-x-4 mt-8 no-print">
            <button @click="editingCompanyInfo = !editingCompanyInfo" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-lg flex items-center transition-colors shadow-md">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
              Modifier en-tête
            </button>
            <button @click="printReport" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg flex items-center transition-colors shadow-md">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M5 4v3H4a2 2 0 00-2 2v3a2 2 0 002 2h1v2a2 2 0 002 2h6a2 2 0 002-2v-2h1a2 2 0 002-2V9a2 2 0 00-2-2h-1V4a2 2 0 00-2-2H7a2 2 0 00-2 2zm8 0H7v3h6V4zm0 8H7v4h6v-4z" clip-rule="evenodd" />
              </svg>
              Imprimer
            </button>
            <button @click="generatePDF(viewingReport.id)" class="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-lg flex items-center transition-colors shadow-md">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="CurrentColor">
                <path fill-rule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm7.707 3.293a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L6.414 9H17a1 1 0 110 2H6.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" />
              </svg>
              Enregistrer en PDF
            </button>
            <button @click="viewingReport = null" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-6 py-3 rounded-lg transition-colors shadow-md">
              Fermer
            </button>
          </div>
        </div>
...
const api = {
  // Interventions
  getInterventions: () => axios.get(`${API_BASE_URL}/interventions`),
  getIntervention: (id) => axios.get(`${API_BASE_URL}/interventions/${id}`),
  ...
  // Autres ressources
  getEquipment: () => axios.get(`${API_BASE_URL}/displayEquipments`),
  getClients: () => axios.get(`${API_BASE_URL}/displayClients`),
...
// Fonction de formatage de date pour l'API
...
const loadInterventions = async () => {
  try {
    ...
    console.log('Interventions chargées:', reports.value)
  } catch (error) {
    console.error('Erreur lors du chargement des interventions:', error)
    alert('Impossible de charger les interventions')
  }
}
...
const loadEquipmentList = async () => {
  try {
    ...
    alert('Impossible de charger la liste des équipements')
  }
}
...
const validateForm = () => {
  ...
  if (!form.value.problem_description) {
    alert('Veuillez saisir la description du problème')
    return false
  }
...
const loadEquipmentDetails = () => {
  ...
  selectedEquipment.value = equipmentList.value.find(e => e.id == form.value.equipement_id)
}
...
const loadClientDetails = () => {
  ...
  selectedClient.value = clientList.value.find(c => c.id == form.value.client_id)
...
const submitForm = async () => {
  try {
    ...
    const dataToSend = {
      ...
      report_date: formatDateForAPI(form.value.report_date),
      contact_time: form.value.contact_time || null,
      status: form.value.status,

      // Options de l'intervention
      is_tested_certified: form.value.is_tested_certified ? 1 : 0,
      is_sent_for_maintenance: form.value.is_sent_for_maintenance ? 1 : 0,
      needs_delivery: form.value.needs_delivery ? 1 : 0,
      delivery_price: form.value.delivery_price || 0,
      under_contract: form.value.under_contract ? 1 : 0,

      // Dates de réparation
      repair_start_date: form.value.repair_start_date ? formatDateForAPI(form.value.repair_start_date) : null,
      repair_end_date: form.value.repair_end_date ? formatDateForAPI(form.value.repair_end_date) : null,
      repair_duration: form.value.repair_duration || '',

      // Diagnostic technique
      technician_findings: form.value.technician_findings || '',

      // Gestion du devis
      quotation_file_name: form.value.quotation_file_name || '',
      quotation_amount: form.value.quotation_amount || 0,
      quotation_email_sent: form.value.quotation_email_sent ? 1 : 0,
      quotation_email_date: form.value.quotation_email_date ? formatDateForAPI(form.value.quotation_email_date) : null,
      quotation_comments: form.value.quotation_comments || '',

      // Signatures
      technician_signature: form.value.technician_signature || '',
      client_signature: form.value.client_signature || '',
    ...
    if (editing.value) {
      response = await api.updateIntervention(viewingReport.value.id, dataToSend)
      interventionId = viewingReport.value.id
      const index = reports.value.findIndex(r => r.id === interventionId)
      if (index !== -1) {
        reports.value[index] = response.data.data
      }
    } else {
      response = await api.createIntervention(dataToSend)
      interventionId = response.data.data.id
      reports.value.push(response.data.data)
    }
    
    // Upload des fichiers si présents (après la création/mise à jour)
    if (form.value.quotation_file) {
      await api.uploadQuotation(interventionId, form.value.quotation_file)
    }
    
    if (form.value.audio_recording_url && form.value.audio_recording_url.startsWith('blob:')) {
      // Convertir l'URL blob en fichier pour l'upload
      const response = await fetch(form.value.audio_recording_url)
      const blob = await response.blob()
      const audioFile = new File([blob], 'enregistrement_audio.webm', { type: 'audio/webm' })
      await api.uploadAudio(interventionId, audioFile)
    }
    
    closeForm()
    loadInterventions()
    alert(editing.value ? 'Intervention mise à jour avec succès' : 'Intervention créée avec succès')
...
const editReport = (report) => {
  ...
  const equip = equipmentList.value.find(e => e.id == report.equipement_id)
  const client = clientList.value.find(c => c.id == report.client_id)
...
const generatePDF = async (id) => {
  try {
    ...
    // Solution de secours : ouvrir une nouvelle fenêtre avec les données
    ...
    viewingReport.value = reports.value.find(r => r.id === id)
    setTimeout(() => {
      window.print()
    }, 500)
  }
...
@media print {
  ...
  /* S'assurer que les signatures s'impriment correctement */
  ...
  /* Améliorer l'affichage des images en impression */
  .print-modal img {
...
/* Améliorer l'affichage des signatures */
img {
  object-fit: contain;
  background: white;
}

/* Style pour les zones de signature vides */
.border-dashed {
