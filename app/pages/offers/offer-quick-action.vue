<template>
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
		...
		<div class="space-y-3">
			<!-- Create New offer -->
			<button @click="createNewoffer"
				class="w-full flex items-center space-x-3 p-3 text-left bg-green-50 hover:bg-green-100 rounded-lg transition-colors group">
				<div
					class="flex-shrink-0 w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center group-hover:bg-green-600 transition-colors">
			...
			<router-link to="/offers"
				...
				<div
					class="flex-shrink-0 w-10 h-10 bg-blue-500 rounded-lg flex items-center justify-center group-hover:bg-blue-600 transition-colors">
					<svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<!-- View Submissions -->
			<router-link to="/offers/submissions"
				...
				<div
					class="flex-shrink-0 w-10 h-10 bg-yellow-500 rounded-lg flex items-center justify-center group-hover:bg-yellow-600 transition-colors">
					<svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<!-- View Projects -->
			<router-link to="/offers/projects"
				...
				<div
					class="flex-shrink-0 w-10 h-10 bg-purple-500 rounded-lg flex items-center justify-center group-hover:bg-purple-600 transition-colors">
					<svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
			...
			<!-- Search offers -->
			<div class="relative">
				<input v-model="searchQuery" @input="performSearch" @focus="showSearchResults = true" @blur="hideSearchResults"
					type="text" placeholder="Rechercher un appel d'offres..."
				...
				<div v-if="showSearchResults && (searchResults.length > 0 || searchQuery.length > 0)"
					...
					<div v-else-if="searchResults.length > 0" class="py-1">
						<div v-for="offer in searchResults" :key="offer.id" @mousedown="selectoffer(offer)"
			...
			<div class="mt-4 pt-4 border-t border-gray-200">
				...
				<div class="grid grid-cols-2 gap-2">
					<button @click="exportData" :disabled="isExporting"
						...
						<svg v-if="isExporting" class="animate-spin h-3 w-3 mr-1 inline" fill="none" stroke="currentColor"
							viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
								d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15">
							</path>
					...
					<label
						class="px-3 py-2 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors cursor-pointer text-center">
						<input ref="fileInput" type="file" accept=".csv,.xlsx,.json" @change="importData" class="hidden">
						Importer
					</label>
				</div>
			</div>
			...
			<div class="mt-4 pt-4 border-t border-gray-200">
				...
				<div class="space-y-2">
					<button @click="downloadTemplate('submission')"
					...
					<button @click="downloadTemplate('financial')"
						class="w-full text-left px-3 py-2 text-xs text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-md transition-colors">
						💰 Modèle financier
					</button>
					<button @click="downloadTemplate('technical')"
						class="w-full text-left px-3 py-2 text-xs text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-md transition-colors">
						🔧 Modèle technique
					</button>
				</div>
...
const getOfferStatusDot = (status: string): string => {
	const colors: Record<string, string> = {
		"En attente de validation": 'bg-gray-400',
		"Validé": 'bg-blue-500',
...
const getOfferStatusBadge = (status: OfferStatus): string => {
	console.log("status:", status);
	const badges: Record<string, string> = {
		"En attente de validation": 'px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800',
		"Validé": 'px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800',
		"Soumis": 'px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800',
		"Rejeté": 'px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800',
	...
	return badges[status] || badges['En attente de validation']!
}
