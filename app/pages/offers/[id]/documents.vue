	...
	<div class="bg-white rounded-lg shadow overflow-hidden">
		...
		<div class="px-6 py-4 border-b border-gray-200 bg-white sticky top-0 z-10">
			<div class="flex items-center justify-end">
				<button
					@click="navigateTo(AppUrl.OFFERS)"
		...
		<div class="p-6">
			<!-- Offer Information -->
			<div class="bg-blue-50 border-blue-600 rounded-lg p-6 mb-6">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<div>
						<label class="block text-sm font-medium text-gray-700"
							>Date limite de soumission</label
						>
						<p class="text-sm text-gray-900">
							{{
								offerData.submission_deadline
									? formatDate(offerData.submission_deadline)
									: ""
							}}
						</p>
					</div>
					<div>
						...
						>
						<p class="text-sm text-gray-900">
							{{ formatCurrency(offerData.estimated_budget) }}
						</p>
					</div>
				</div>
			</div>
			...
			<!-- <div class="p-4 border rounded border-emerald-600 bg-emerald-50 mb-6">
				<div class="flex items-center justify-between">
					<div>
						<h3 class="text-lg font-medium text-gray-900">
							État de disponibilité des documents
						</h3>
						<p class="text-sm text-gray-600 mt-1">
							Vérifiez le statut de chaque document requis pour votre soumission
						</p>
					</div>
					<button
			...
			<div class="space-y-4">
				<div
					v-for="(document, index) in documentsAvailability"
					:key="index"
					...
					<div class="flex items-start justify-between">
						<div class="flex items-start space-x-4 flex-1">
							<div class="flex-shrink-0 mt-1">
								...
								<div
									v-else-if="document.status === 'missing'"
							...
							<div class="flex-1">
								...
								<div
									v-if="document.isAvailable && document.isValid === false"
									class="mt-3"
								>
									<label class="block text-sm font-medium text-gray-700 mb-1"
										>Date limite de demande</label
									>
									<input
										type="date"
								...
								<div v-if="document.isAvailable" class="mt-4">
									...
									>
									<div class="space-y-2">
										<!-- Image existante du type de document -->
										<div
											v-if="document.file?.path"
											class="flex items-center space-x-4 p-3 bg-blue-50 border border-blue-200 rounded-md"
										>
											<img
												:src="storageUrl(document.file.path)"
												:alt="document.file.name || document.name"
												class="h-20 w-20 object-cover rounded-md border border-gray-200"
											/>
											<div class="flex-1">
												<span class="text-sm font-medium text-blue-800">{{
													document.file.name || "Fichier"
												}}</span>
											</div>
										</div>
										<!-- Fichier uploadé localement -->
										<div
											v-else-if="document.uploadedFile"
										...
										>
											<div class="flex items-center space-x-3">
												...
												<span class="text-sm font-medium text-green-800">{{
													document.uploadedFile?.name || "Fichier"
												}}</span>
											</div>
											<button
		...
		<div v-if="false" class="fixed inset-0 z-50 overflow-y-auto">
			<div
						...
						<div class="sm:flex sm:items-start">
							...
							<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
								...
								<div class="space-y-4">
									...
									<div
										v-if="
											newDocument.isAvailable && newDocument.isValid === false
										"
									>
										<label class="block text-sm font-medium text-gray-700 mb-1"
											>Date limite de demande</label
...
import { defaultOfferData, type Offer } from "~/models/Offer";
...
useHead({ title: "Disponibilité des documents" });

const offerStore = useOfferStore();
const documentTypeStore = useDocumentTypeStore();

const { documentTypes } = storeToRefs(documentTypeStore);
...
// Documents availability status
const documentsAvailability = ref<DocumentType[]>([]);
...
const availableCount = computed(() => {
	return documentsAvailability.value.filter(
		(doc) => doc.isAvailable && doc.isValid,
	).length;
});

const inProgressCount = computed(() => {
	return documentsAvailability.value.filter(
		(doc) => doc.isAvailable && doc.isValid === false,
	).length;
});
...
const loadOfferData = async () => {
	try {
		...
		Swal.fire({
			title: "Cet appel d'offre n'a été trouvé",
			icon: "error",
			...
			if (_.dismiss === Swal.DismissReason.cancel) {
				router.go(-1);
			}
		});
	} finally {
...
const loadDocuments = async () => {
	try {
		...
		documentsAvailability.value = documentTypes.value;
		// documentsAvailability.value = documentTypes.value.map((doc: any) => ({
		// 	name: doc.name,
		...
		// 	isValid: doc.isValid,
		// 	requestDeadline: doc.requestDeadline,
		// 	file: doc.file ?? null,
		// 	isCustom: false,
		// 	uploadedFile: null,
		// }));
...
const updateDocumentStatus = (index: number, status: DocumentStatusEnum) => {
...
const updateDocumentAvailability = (index: number) => {
	...
	if (doc.isAvailable) {
		doc.status = doc.isValid
			? DocumentStatusEnum.AVAILABLE
			: DocumentStatusEnum.IN_PROGRESS;
	} else {
		doc.status = DocumentStatusEnum.MISSING;
		doc.isValid = null;
...
const addNewDocument = () => {
	if (!newDocument.value.name.trim()) {
		Swal.fire({
			icon: "error",
			text: "Veuillez saisir le nom du document",
		});
		return;
	}
	...
	const document: DocumentType = {
		...
		status: newDocument.value.isAvailable
			? newDocument.value.isValid
				? DocumentStatusEnum.AVAILABLE
				: DocumentStatusEnum.IN_PROGRESS
...
const handleFileUpload = (event: Event, index: number) => {
	...
	if (file.size > 10 * 1024 * 1024) {
		Swal.fire({
			icon: "error",
			text: "Le fichier est trop volumineux. Taille maximum : 10MB",
		});
		return;
	}
	...
	if (!allowedTypes.includes(file.type)) {
		Swal.fire({
			icon: "error",
			text: "Type de fichier non autorisé. Formats acceptés : PDF, DOC, DOCX, JPG, PNG",
		});
		return;
	}
...
const saveProgress = async () => {
	try {
		...
		Swal.fire({
			icon: "success",
			text: "Progrès sauvegardé avec succès",
		});
	} catch (error) {
