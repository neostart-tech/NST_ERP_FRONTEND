<template>
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6" role="main">
		<div class="flex items-center justify-between mb-6">
			<div>
				<h1 class="text-2xl font-bold text-gray-900">Documents des appels d'offres</h1>
				<p class="text-sm text-gray-600 mt-1">Sélectionnez un appel d'offre pour gérer ses documents.</p>
			</div>
		...
		<div v-else-if="sortedOffers.length === 0" class="py-10">
			<EmptyState
				...
				description="Aucun appel d'offre disponible pour gérer les documents."
				...
				<tbody class="bg-white divide-y divide-gray-200">
					<tr v-for="offer in sortedOffers" :key="offer.id" class="hover:bg-gray-50">
						...
						<td class="px-6 py-4 text-right">
							<NuxtLink
								:to="AppUrl.parameterize(AppUrl.OFFERS_DOCUMENTS, offer.id)"
								class="inline-flex items-center px-3 py-2 rounded-md text-sm text-white bg-emerald-600 hover:bg-emerald-700"
							>
								Gérer
							</NuxtLink>
						</td>
