    ...
    <div>
      <div class="flex items-center gap-10">
        ...
        <div>
          ...
          <input type="number" v-model.number="nombreEcheances"  min="1" max="12" class="border border-gray-300 rounded w-full p-2 focus:ring-2 focus:ring-indigo-500" />
        </div>
      </div>

      <!-- Aperçu échéancier modifiable -->
      <div v-if="factureSelectionnee && apercu.length > 0" class="bg-gray-50 p-4 rounded-lg mt-4">
        <h2 class="font-semibold mb-2">Aperçu de l'échéancier</h2>
        <div class="mb-2">
          <span class="font-medium">Total restant : </span>
          {{ formatCurrency(totalRestant) }}
          <span v-if="Math.abs(totalRestant) > 0.01" class="text-red-500 ml-2">
            (La somme des échéances ne correspond pas au total)
          </span>
        </div>
        <table class="min-w-full border">
          ...
          <tbody>
            <tr v-for="(echeance, index) in apercu" :key="index">
              <td class="border border-gray-400 text-center px-4 py-2">
                {{ index + 1 }}
              </td>
              <td class="border border-gray-400 px-4 py-2 text-center ">
                <input type="number" v-model.number="echeance.montant" @input="ajusterMontants(index)"
                  class="bg-gray-50 text-center"/>
              </td>
              <td class="border border-gray-400 px-4 py-2 text-center">
                <input type="date" v-model="echeance.date"
                  class="bg-gray-50 text-center"/>
              </td>
            </tr>
          </tbody>
...
// Calcul du total restant (somme des échéances - total facture)
const totalRestant = computed(() => {
  if (!factureSelectionnee.value) return 0;
  const totalEcheances = apercu.value.reduce((sum, e) => sum + Number(e.montant || 0), 0);
  return factureSelectionnee.value.total - totalEcheances;
});

// Générer l'aperçu quand les paramètres changent
...
function genererApercu() {
  ...
  const montantInitial = factureSelectionnee.value.total / nombreEcheances.value;
  ...
  apercu.value = Array.from({ length: nombreEcheances.value }, (_, i) => {
    ...
    return {
      montant: i === nombreEcheances.value - 1 
        ? factureSelectionnee.value.total - (montantInitial * i) // Dernière échéance ajustée
        : montantInitial,
      date: formattedDate
    };
  });
...
function ajusterMontants(indexModifie) {
  ...
  // Calculer le total des échéances précédentes (celles qu'on ne modifie pas)
  let totalPrecedent = 0;
  for (let i = 0; i <= indexModifie; i++) {
    totalPrecedent += Number(apercu.value[i].montant) || 0;
  }

  // Calculer le reste à répartir sur les échéances suivantes
  ...
  if (nbEcheancesRestantes > 0) {
    ...
    // Ajuster seulement les échéances suivantes
  ...
  const totalActuel = apercu.value.reduce((sum, e) => sum + Number(e.montant || 0), 0);
 ...
 async function validerEcheancier() {
  if (Math.abs(totalRestant.value) > 0.01) {
    // alert("La somme des échéances ne correspond pas au total de la facture.");
    Swal.fire({
      icon:'warming',
      title:'Erreur',
  ...
  try{
    ...
    // alert( "Echeancier validé et enregistré avec succès")
    Swal.fire({
      icon:'success',
      title:'Succès',
      text:"Echeancier validé et enregistré avec succès",
      timer:2000,
