<template>
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
		...
		<template v-if="filteredProducts.length > 0">
					...
					>
						...
						<div class="flex items-start justify-between mb-3">
							<div class="flex items-center flex-1">
								<div
									class="flex-shrink-0 h-12 w-12 bg-blue-100 rounded-md flex items-center justify-center"
								>
									{{ index + 1 }}
								</div>
						...
						<!-- Actions -->
						<div
							class="flex justify-end space-x-2 pt-3 border-t border-gray-100"
						>
							<button
			...
			<div
				class="hidden xl:block overflow-x-auto rounded-lg border border-gray-200"
			>
						...
						>
							<td class="px-6 py-4 whitespace-nowrap">
								<div class="flex items-center">
									<div
										class="flex-shrink-0 text-smf h-10 w-10 bg-blue-100 rounded-md flex items-center justify-center"
									>
										{{ ++index }}
									</div>
									<div class="ml-4">
		...
		<!-- EmptyState -->
		<div v-else-if="isLoading || !products || filteredProducts.length === 0">
			<EmptyState
				title="Aucun produit trouvé"
				description="Il n'y a actuellement aucun produit à afficher."
				icon="heroicons:cube"
...
import { productFormData, type Product } from "~/models/Product";
...
const deleteProduct = async (product: Product) => {
	Swal.fire({
		...
		html: `Voulez-vous supprimer le produit <strong>${product.name}</strong>?`,
		...
		if (result.isConfirmed) {
			try {
				...
				Swal.fire({
					title: "Susppression effectuée avec succès",
					icon: "success",
				});
			} catch (error) {
				Swal.fire({
...
const paginatedProducts = computed(() => {
	return filteredProducts.value.slice(range.start - 1, range.end);
});
</script>
