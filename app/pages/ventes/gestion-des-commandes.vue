	...
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
		...
		<template v-if="orders.length > 0">
			...
			<div class="lg:hidden grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
				<div v-for="proforma in paginatedOrders" :key="proforma.id + '-card'"
					...
					<div class="p-4">
						<div class="flex items-start justify-between mb-3">
							...
							<span :class="resolveStatusColor(proforma.status)" class="px-2 py-1 text-xs font-semibold rounded-full">
						...
						<div class="mt-3 space-y-2">
							<div class="text-sm text-gray-600">
								<span class="font-medium">Objet:</span> {{ proforma.comment || "-" }}
							</div>
						...
						<div class="flex justify-between mt-4 pt-3 border-t border-gray-100">
							<div class="flex gap-2">
								<button v-if="proforma.status === 'pending'" @click.stop="updateStatus(proforma, 'delivered')"
									class="p-1.5 text-green-600 hover:bg-green-50 rounded-full" title="Valider">
									<Icon name="heroicons:check-circle" class="w-5 h-5" />
								</button>
								<button v-if="proforma.status === 'pending'" @click.stop="updateStatus(proforma, 'canceled')"
...
const computeStats = () => {
	...
	stats.value.pending = items.filter(_ => _.status === "pending").length;
	stats.value.validated = items.filter(_ => _.status === "delivered").length;
	stats.value.canceled = items.filter(_ => _.status === "canceled").length;
};
