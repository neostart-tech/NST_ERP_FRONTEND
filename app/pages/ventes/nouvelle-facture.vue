<template>
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
				...
				<div class="w-full">
					...
					<select
						...
						<option :value="InvoiceTypeEnum.DEFINITIVE">{{ InvoiceTypeEnum.DEFINITIVE }}</option>
						<option :value="InvoiceTypeEnum.ADVANCE">{{ InvoiceTypeEnum.ADVANCE }}</option>
					</select>
				</div>
		...
		<div v-else-if="order">
			...
			<div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg" v-if="orderInfo !== null">
			...
			<div v-if="factureType === InvoiceTypeEnum.ADVANCE && orderArticles.length > 0"
			...
			<div v-if="orderArticles.length > 0 && factureType" class="mt-6 flex justify-end">
				<div class="w-full max-w-sm space-y-3 bg-gray-50 p-4 rounded-lg border border-gray-200">
					...
					<div v-if="factureType === 'Facture définitive'">
					...
					<div v-if="factureType === InvoiceTypeEnum.ADVANCE">
						<div class="flex justify-between text-sm text-gray-600">
							<span class="font-medium">Total HT :</span>
							<span class="font-semibold">{{ formatCurrency(subtotal) }}</span>
						</div>
						...
						<div class="flex justify-between text-lg font-bold text-gray-900 pt-3 border-t-2 border-gray-300 mt-3">
							...
							<span class="text-blue-600">{{ (acompteTTC) }}</span>
						</div>
					</div>
				</div>
			</div>
...
import {InvoiceTypeEnum, type Order} from '~/models/Invoice'
import {useRouter} from 'vue-router'
import {useInvoiceTypeStore} from '@/stores/Sale/InvoiceTypeStore'
import {useInvoiceStore} from '@/stores/Sale/InvoiceStore'
import OrderComboBox from "@/components/ui/OrderComboBox.vue";

useHead({title: "Nouvelle Facture"});
...
const factureType = ref<string | InvoiceTypeEnum>('')
...
const submitInvoice = async () => {
	...
	const payload = {
		...
		acomptePercent: factureType.value === InvoiceTypeEnum.ADVANCE ? acomptePercent.value : null,
	...
	try {
		...
		router.push(AppUrl.INVOICES_INDEX);
	} catch (error) {
		Swal.fire({
			icon: 'error',
			title: 'Erreur',
