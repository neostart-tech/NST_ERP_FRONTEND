<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <!-- Carte Total -->
      <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200 p-5 shadow-sm hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-blue-800">Total Proformas</p>
            <p class="text-2xl font-bold text-blue-900 mt-1">{{ formatNumber(stats.total) }}</p>
          ...
          <div class="p-2.5 rounded-lg bg-blue-500 bg-opacity-20">
      ...
      <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl border border-amber-200 p-5 shadow-sm hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-amber-800">Brouillons</p>
            <p class="text-2xl font-bold text-amber-900 mt-1">{{ formatNumber(stats.draft) }}</p>
          ...
          <div class="p-2.5 rounded-lg bg-amber-500 bg-opacity-20">
            <Icon name="heroicons:document-duplicate" class="w-6 h-6 text-amber-600" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl border border-green-200 p-5 shadow-sm hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-green-900 mt-1">{{ formatNumber(stats.validated) }}</p>
      ...
      <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-xl border border-red-200 p-5 shadow-sm hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold text-red-900 mt-1">{{ formatNumber(stats.rejected) }}</p>
    ...
    <!-- Tableau des dernières proformas -->
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
      <div class="p-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-800">Dernières proformas</h3>
      </div>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(proforma, index) in latestProformas" :key="index" class="hover:bg-gray-50">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                {{ proforma.reference || 'N/A' }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ proforma.client?.name || 'N/A' }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  :class="getStatusBadgeClass(proforma.status)"
            ...
            <tr v-if="latestProformas.length === 0">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p class="text-sm text-gray-700">
              Affichage de <span class="font-medium">1</span> à <span class="font-medium">5</span> sur <span class="font-medium">{{ latestProformas.length }}</span> résultats
            </p>
          </div>
          <div>
...
// Récupérer les 5 dernières proformas
...
const pieData = ref({
  labels: ['Brouillons', 'Validés', 'Rejetés'],
  datasets: [
    {
      label: 'Proformas',
      data: [],
...
const computeStats = () => {
  ...
  proformas.value.forEach(proforma => {
    const month = new Date(proforma.date).getMonth()
    monthlyData[month]++
  })
